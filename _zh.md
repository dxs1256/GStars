# ⭐ GitHub Stars Index

> Updated: 2026-06-23 12:28 UTC · Total: 108

---

## 📑 Index
- [[#iblogc/GithubStarsIndex|iblogc/GithubStarsIndex]]
- [[#AmintaCCCP/GithubStarsManager|AmintaCCCP/GithubStarsManager]]
- [[#chenxuuu/sms_forwarding|chenxuuu/sms_forwarding]]
- [[#agentscope-ai/QwenPaw|agentscope-ai/QwenPaw]]
- [[#Teminuosi/3x-ui|Teminuosi/3x-ui]]
- [[#MoonTechLab/Selene-TV|MoonTechLab/Selene-TV]]
- [[#nianzhibai/91|nianzhibai/91]]
- [[#igareck/vpn-configs-for-russia|igareck/vpn-configs-for-russia]]
- [[#truelockmc/streambert|truelockmc/streambert]]
- [[#hureyqi/x-spider-mod-2026|hureyqi/x-spider-mod-2026]]
- [[#avacocloud/XHTTP-Installer|avacocloud/XHTTP-Installer]]
- [[#XCQ0607/lxserver|XCQ0607/lxserver]]
- [[#tmxk2020/abc|tmxk2020/abc]]
- [[#Eric-Yibo-Shen/zhangxuefeng-skillset|Eric-Yibo-Shen/zhangxuefeng-skillset]]
- [[#alchaincyf/zhangxuefeng-skill|alchaincyf/zhangxuefeng-skill]]
- [[#katelya77/K-Vault|katelya77/K-Vault]]
- [[#qianlong520/Telegram_MistRelay|qianlong520/Telegram_MistRelay]]
- [[#Panniantong/Agent-Reach|Panniantong/Agent-Reach]]
- [[#shuaiplus/nodewarden|shuaiplus/nodewarden]]
- [[#qaz741wsd856/warden-worker|qaz741wsd856/warden-worker]]
- [[#develop202/kgcheckin|develop202/kgcheckin]]
- [[#XianYuDaXian/iptv-m3u-manager|XianYuDaXian/iptv-m3u-manager]]
- [[#gaoyaxuan/luci-app-pushbot|gaoyaxuan/luci-app-pushbot]]
- [[#emojiiii/magekit-app|emojiiii/magekit-app]]
- [[#lesnolie/movecar|lesnolie/movecar]]
- [[#lostmaniac/air780e_forwarder|lostmaniac/air780e_forwarder]]
- [[#jianzhichu/dysync.net|jianzhichu/dysync.net]]
- [[#hezhizheng/go-wxpush|hezhizheng/go-wxpush]]
- [[#ieax/renewhelper|ieax/renewhelper]]
- [[#dushixiang/uart_sms_forwarder|dushixiang/uart_sms_forwarder]]
- [[#vodtv/api|vodtv/api]]
- [[#ngosang/trackerslist|ngosang/trackerslist]]
- [[#yonggekkk/argosbx|yonggekkk/argosbx]]
- [[#byJoey/cfnew|byJoey/cfnew]]
- [[#avwo/whistle|avwo/whistle]]
- [[#lanyeeee/bilibili-video-downloader|lanyeeee/bilibili-video-downloader]]
- [[#youhunwl/TVAPP|youhunwl/TVAPP]]
- [[#xyfqzy/free-nodes|xyfqzy/free-nodes]]
- [[#hyue418/go-rename|hyue418/go-rename]]
- [[#btjawa/BiliTools|btjawa/BiliTools]]
- [[#GitHubDaily/GitHubDaily|GitHubDaily/GitHubDaily]]
- [[#proxifly/free-proxy-list|proxifly/free-proxy-list]]
- [[#1c7/chinese-independent-developer|1c7/chinese-independent-developer]]
- [[#fish2018/pansou|fish2018/pansou]]
- [[#Usagi-org/ai-goofish-monitor|Usagi-org/ai-goofish-monitor]]
- [[#ring1012/r1-iot-java|ring1012/r1-iot-java]]
- [[#OpenListTeam/OpenList|OpenListTeam/OpenList]]
- [[#TapXWorld/ChinaTextbook|TapXWorld/ChinaTextbook]]
- [[#krau/SaveAny-Bot|krau/SaveAny-Bot]]
- [[#yt-dlp/yt-dlp|yt-dlp/yt-dlp]]
- [[#thinkoaa/Deadpool|thinkoaa/Deadpool]]
- [[#BCUninstaller/Bulk-Crap-Uninstaller|BCUninstaller/Bulk-Crap-Uninstaller]]
- [[#imsyy/DailyHotApi|imsyy/DailyHotApi]]
- [[#TheSpeedX/PROXY-List|TheSpeedX/PROXY-List]]
- [[#sunny9577/proxy-scraper|sunny9577/proxy-scraper]]
- [[#adminouyang/231006|adminouyang/231006]]
- [[#lyswhut/lx-music-mobile|lyswhut/lx-music-mobile]]
- [[#breezy-weather/breezy-weather|breezy-weather/breezy-weather]]
- [[#guoyue2010/lxmusic-|guoyue2010/lxmusic-]]
- [[#lalifeier/IPTV|lalifeier/IPTV]]
- [[#cngege/GitHubDesktop2Chinese|cngege/GitHubDesktop2Chinese]]
- [[#Evil0ctal/Douyin_TikTok_Download_API|Evil0ctal/Douyin_TikTok_Download_API]]
- [[#cxasm/notepad--|cxasm/notepad--]]
- [[#521xueweihan/HelloGitHub|521xueweihan/HelloGitHub]]
- [[#bia-pain-bache/BPB-Worker-Panel|bia-pain-bache/BPB-Worker-Panel]]
- [[#VirtualHotBar/HotPEToolBox|VirtualHotBar/HotPEToolBox]]
- [[#VirtualHotBar/NetMount|VirtualHotBar/NetMount]]
- [[#ZCShou/GoGoGo|ZCShou/GoGoGo]]
- [[#Guovin/iptv-api|Guovin/iptv-api]]
- [[#yonggekkk/Cloudflare-vless-trojan|yonggekkk/Cloudflare-vless-trojan]]
- [[#dr34m-cn/taosync|dr34m-cn/taosync]]
- [[#1234567Yang/cf-proxy-ex|1234567Yang/cf-proxy-ex]]
- [[#peasoft/NoMoreWalls|peasoft/NoMoreWalls]]
- [[#qist/tvbox|qist/tvbox]]
- [[#NextAlone/Nagram|NextAlone/Nagram]]
- [[#httpsok/httpsok|httpsok/httpsok]]
- [[#zhuima/awesome-cloudflare|zhuima/awesome-cloudflare]]
- [[#sandboxie-plus/Sandboxie|sandboxie-plus/Sandboxie]]
- [[#spiritLHLS/ecs|spiritLHLS/ecs]]
- [[#wanghongenpin/proxypin|wanghongenpin/proxypin]]
- [[#Barabama/FreeNodes|Barabama/FreeNodes]]
- [[#mfbpn/mfbpn_nodes|mfbpn/mfbpn_nodes]]
- [[#alantang1977/X|alantang1977/X]]
- [[#8680/GOODBYEADS|8680/GOODBYEADS]]
- [[#n3rddd/N3RD|n3rddd/N3RD]]
- [[#217heidai/adblockfilters|217heidai/adblockfilters]]
- [[#JoeanAmier/TikTokDownloader|JoeanAmier/TikTokDownloader]]
- [[#yonggekkk/x-ui-yg|yonggekkk/x-ui-yg]]
- [[#mediago-dev/mediago|mediago-dev/mediago]]
- [[#biliup/biliup|biliup/biliup]]
- [[#lyswhut/lx-music-sync-server|lyswhut/lx-music-sync-server]]
- [[#1Panel-dev/1Panel|1Panel-dev/1Panel]]
- [[#privacy-protection-tools/anti-AD|privacy-protection-tools/anti-AD]]
- [[#yuliskov/SmartTube|yuliskov/SmartTube]]
- [[#leiurayer/downkyi|leiurayer/downkyi]]
- [[#dgtlmoon/changedetection.io|dgtlmoon/changedetection.io]]
- [[#fangzesheng/free-api|fangzesheng/free-api]]
- [[#easychen/rsspush|easychen/rsspush]]
- [[#LogicJake/WebMonitor|LogicJake/WebMonitor]]
- [[#XIU2/TrackersListCollection|XIU2/TrackersListCollection]]
- [[#filecxx/FileCentipede|filecxx/FileCentipede]]
- [[#hongyonghan/Docker_Microsoft365_E5_Renew_X|hongyonghan/Docker_Microsoft365_E5_Renew_X]]
- [[#lyswhut/lx-music-desktop|lyswhut/lx-music-desktop]]
- [[#BluePointLilac/ContextMenuManager|BluePointLilac/ContextMenuManager]]
- [[#bclswl0827/v2ray-heroku|bclswl0827/v2ray-heroku]]
- [[#ShareX/ShareX|ShareX/ShareX]]
- [[#zhaoolee/ChromeAppHeroes|zhaoolee/ChromeAppHeroes]]
- [[#2dust/v2rayN|2dust/v2rayN]]

---

## iblogc/GithubStarsIndex

> [!info]
> ⭐ 75 · Jinja · 2026-06-23T03:13:11Z  
> [GitHub](https://github.com/iblogc/GithubStarsIndex) · [Website](https://stars.iblogc.com)  
> `#AI 智能体` `#GitHub Actions` `#知识管理` `#自动化运维` 
> GitHubStarsIndex 是一款基于 AI 的自动化知识管理工具，自动抓取 GitHub Stars 并生成智能摘要。它具备标签归一化治理、并发处理及增量更新机制，支持同步至 Obsidian 和 GitHub Pages，帮助开发者高效构建个人技术知识库。
> <sub>自动抓取 GitHub Stars，生成 AI 摘要，便于检索。</sub>

---
## AmintaCCCP/GithubStarsManager

> [!info]
> ⭐ 3,094 · TypeScript · 2026-06-23T15:43:56Z  
> [GitHub](https://github.com/AmintaCCCP/GithubStarsManager) · [Website](https://gsm.aminta.top/)  
> `#AI智能体` `#代码管理` `#桌面应用` 
> GithubStarsManager 是一款基于 Electron 的跨平台桌面应用，解决 GitHub Star 仓库管理难题。它利用 AI 技术实现仓库自动分类、语义搜索和 README 摘要，告别收藏即遗忘。支持 Release 跟踪、一键下载及 Gist 管理，数据 100% 本地存储，兼顾隐私与高效，是提升代码资产检索效率的必备工具。
> <sub>An app for managing github starred repositories.</sub>

---
## chenxuuu/sms_forwarding

> [!info]
> ⭐ 1,211 · C++ · 2026-06-23T20:04:50Z  
> [GitHub](https://github.com/chenxuuu/sms_forwarding) · [Website](http://sms.j2.cx/)  
> `#ESP32` `#嵌入式开发` `#物联网` `#短信转发` 
> 本项目是一款基于ESP32C3与ML307R的低成本硬件短信转发器，支持移动/联通/电信卡。通过Web界面管理，可实现短信自动转发至邮箱、钉钉、飞书及多种推送服务。具备长短信合并、远程重启及余额消耗功能，是极佳的物联网通知与保号工具。
> <sub>超低成本硬件短信转发器，支持移动联通电信，ML307R+ESP32C3</sub>

---
## agentscope-ai/QwenPaw

> [!info]
> ⭐ 19,930 · Python · 2026-06-23T19:23:33Z  
> [GitHub](https://github.com/agentscope-ai/QwenPaw) · [Website](http://qwenpaw.agentscope.io/)  
> `#AI智能体` `#个人助理` `#技能扩展` `#本地部署` `#agent` `#agent-harness` `#agentscope` `#harness-engineering` `#llm-tools` `#llms` `#skills` `#super-agent` 
> QwenPaw 是一款由 AgentScope 团队打造的个人 AI 助手，支持本地或云端灵活部署，确保数据隐私完全可控。它具备强大的技能扩展能力，可集成 PDF/Office 处理、新闻摘要及多聊天应用交互。作为开源项目，它易于安装且高度可定制，旨在为用户提供安全、高效且随需而变的智能工作流支持。
> <sub>Your Personal AI Assistant; easy to install, deploy on your own machine or on the cloud; supports multiple chat apps with easily extensible capabilities.</sub>

---
## Teminuosi/3x-ui

> [!info]
> ⭐ 102 · TypeScript · 2026-06-23T10:11:16Z  
> [GitHub](https://github.com/Teminuosi/3x-ui)  
> `#Xray-core` `#一键部署` `#代理管理` `#网络工具` 
> 3x-ui 是 Xray-core 的网页控制面板，主打极简配置。内置 VLESS Reality 等一键模板，支持中转分流、多服务器批量管理及扫码导入。界面中文化，支持 SQLite/PostgreSQL 数据库，适合快速搭建和管理代理节点，让协议配置更简单高效。

---
## MoonTechLab/Selene-TV

> [!info]
> ⭐ 488 · N/A · 2026-06-23T04:18:51Z  
> [GitHub](https://github.com/MoonTechLab/Selene-TV)  
> `#Android TV` `#Jetpack Compose` `#流媒体聚合` `#视频播放器` 
> Selene-TV 是一款专为 Android TV 打造的沉浸式视频客户端，基于 Kotlin 与 Jetpack Compose for TV 构建。它支持多源聚合搜索、双内核播放及跨平台弹幕聚合，内置手机扫码遥控功能，深度适配大屏遥控器操作，提供影院级观影体验。
> <sub>以 MoonTV v100 版本 / Helios 为后端的 Android TV 客户端，针对遥控器设备优化</sub>

---
## nianzhibai/91

> [!info]
> ⭐ 1,181 · Go · 2026-06-23T16:43:46Z  
> [GitHub](https://github.com/nianzhibai/91)  
> `#Docker部署` `#媒体库管理` `#私有部署` `#网盘集成` 
> 91 是一款个人私有视频站部署工具，支持多网盘后端（115/PikPak/OneDrive/Google Drive等）及本地存储。核心亮点包括302低带宽播放模式、自动封面与预览生成、爬虫脚本导入及短视频沉浸模式。提供一键脚本和 Docker Compose 两种部署方式，适合个人搭建私有媒体库。
> <sub>nine one</sub>

---
## igareck/vpn-configs-for-russia

> [!info]
> ⭐ 7,153 · N/A · 2026-06-23T19:57:06Z  
> [GitHub](https://github.com/igareck/vpn-configs-for-russia)  
> `#代理协议` `#绕过封锁` `#网络工具` `#自动化运维` `#free-vpn-key` `#free-vpn-keys` `#free-vpn-russia` `#roskomnadzor` `#russia-vpn` `#shadowsocks` `#shadowsocks-free` `#v2ray` `#vless` `#vless-reality` `#vpn` `#vpn-config` `#vpn-configuration` `#vpn-for-russia` `#vpn-free-russia` `#vpn-key` `#vpn-keys` `#vpn-russia` `#whitelist` `#xray` 
> 专为俄罗斯网络环境打造的免费 VPN 配置仓库，支持 VLESS、Trojan 等主流协议。通过自动化脚本每 2 小时进行可用性测试与更新，精准过滤无效节点，提供 TXT 订阅链接，兼容主流客户端，助力用户稳定绕过网络封锁。
> <sub>🗽Бесплатные и проверенные VPN конфигурации, работающие в РФ ⚪ Белые списки / обход белых списков ⚪ Free and checked VPN configurations that work in Russia ⚪ Whitelists bypass</sub>

---
## truelockmc/streambert

> [!info]
> ⭐ 5,455 · JavaScript · 2026-06-23T19:49:39Z  
> [GitHub](https://github.com/truelockmc/streambert)  
> `#Electron 应用` `#数据抓取` `#流媒体工具` `#视频下载` `#anime` `#anime-downloader` `#anime-scraper` `#downloader` `#electron` `#modern-ui` `#movies` `#movies-streaming` `#opinionated` `#piracy` `#series` `#streaming` `#streaming-video` `#tmdb-api` `#tv` 
> Streambert 是一款基于 Electron 的跨平台桌面应用，聚合全球电影、剧集与动漫资源。它整合 VidSrc 与 TMDB 等多源数据，支持字幕管理及多线程下载。核心优势在于完全无广告、无追踪，保护用户隐私，并提供高度可定制界面，是追求纯净观影体验用户的理想选择。
> <sub>A cross-platform Electron Desktop App to stream and download any Movie, TV Series or Anime in the World. Zero Ads and Tracking</sub>

---
## hureyqi/x-spider-mod-2026

> [!info]
> ⭐ 103 · TypeScript · 2026-06-19T09:05:19Z  
> [GitHub](https://github.com/hureyqi/x-spider-mod-2026)  
> `#AI 辅助开发` `#批量下载工具` `#社交媒体爬虫` 
> 本项目是 X-Spider 的增强修改版，专注于推特媒体（图片/视频）的高效批量下载。支持日期过滤、自动去重及代理配置，解决原作者停更问题，现由 AI 辅助持续维护，确保工具稳定易用。

---
## avacocloud/XHTTP-Installer

> [!info]
> ⭐ 859 · HTML · 2026-06-23T00:04:08Z  
> [GitHub](https://github.com/avacocloud/XHTTP-Installer)  
> `#Xray` `#网络代理` `#自动化部署` `#边缘计算` 
> 该工具是一款专为 Ubuntu 设计的自动化部署脚本，利用 Vercel 或 Netlify 边缘计算平台作为 CDN 中继，构建 VLESS+XHTTP+TLS 安全通道。它通过隐藏源站 IP 显著提升抗封锁能力，支持一键安装、自动 SSL 证书配置及智能故障修复，是搭建高隐蔽性网络基础设施的优选。
> <sub>VLESS+XHTTP+TLS Auto-Installer for Ubuntu — Vercel / Netlify relay</sub>

---
## XCQ0607/lxserver

> [!info]
> ⭐ 580 · JavaScript · 2026-06-23T08:47:17Z  
> [GitHub](https://github.com/XCQ0607/lxserver) · [Website](https://xcq0607.github.io/lxserver/)  
> `#Web 播放器` `#数据同步` `#私有云` `#音乐服务` 
> 这是一个 LX Music 增强版数据同步服务端，内置功能强大的 Web 播放器。支持多源聚合搜索、歌单管理、歌词卡片分享及 Subsonic 协议。提供现代化 UI、自动化缓存和自定义源管理，适合搭建个人私有音乐云，实现跨设备无缝同步与网页听歌体验。
> <sub>LX Music 数据同步服务端，同时支持了部分lx-music-desktop的网页端实现。</sub>

---
## tmxk2020/abc

> [!info]
> ⭐ 368 · N/A · 2026-06-23T09:29:45Z  
> [GitHub](https://github.com/tmxk2020/abc)  
> `#Cloudflare Worker` `#Docker部署` `#流媒体聚合` `#订阅管理` 
> 本项目提供视频流媒体订阅资源的免 Workers 部署方案，支持 Docker 一键部署与 Cloudflare Worker 脚本。兼容 OK影视、DIYP 等客户端，提供 TXT/M3U 订阅格式及聚合搜索功能，通过动态变量值管理防止资源滥用，适合个人学习与技术交流使用。

---
## Eric-Yibo-Shen/zhangxuefeng-skillset

> [!info]
> ⭐ 359 · N/A · 2026-06-23T16:16:36Z  
> [GitHub](https://github.com/Eric-Yibo-Shen/zhangxuefeng-skillset)  
> `#AI 智能体` `#RAG 平台` `#决策支持` `#提示工程` 
> 本项目是张雪峰高考志愿填报的开源决策知识库，旨在消除普通家庭信息差。核心涵盖专业选择、志愿填报等 8 大模块。支持一键部署为 OpenClaw Skills 接入微信，或导入 Dify 等 RAG 平台构建 AI 顾问，提供科学决策框架而非简单数据匹配。

---
## alchaincyf/zhangxuefeng-skill

> [!info]
> ⭐ 8,689 · N/A · 2026-06-23T17:29:26Z  
> [GitHub](https://github.com/alchaincyf/zhangxuefeng-skill)  
> `#AI 智能体` `#Agent Skills` `#职业规划` `#认知框架` 
> 本项目构建了张雪峰的认知操作系统，将高考志愿、考研及职业规划经验转化为可运行的思维框架。基于 Agent Skills 标准开发，兼容多 AI 运行时，提炼核心心智模型与决策启发式，为用户提供具备实战逻辑的升学与职业发展咨询方案。
> <sub>张雪峰.skill — 张雪峰的认知操作系统。高考志愿/考研/职业规划的实战思维框架。由女娲.skill生成。</sub>

---
## katelya77/K-Vault

> [!info]
> ⭐ 672 · JavaScript · 2026-06-23T15:46:58Z  
> [GitHub](https://github.com/katelya77/K-Vault)  
> `#云存储` `#文件托管` `#无服务器` `#私有云盘` 
> K-Vault 是一款基于 Cloudflare Serverless 的聚合云盘方案，支持 Telegram、R2、S3 等多存储后端。通过双模部署实现零成本构建私有数据金库，具备在线预览、分片上传及 API 认证功能，是个人文件托管与图床的理想选择。
> <sub>基于 Cloudflare 的 Serverless 聚合云盘。以 Telegram 为核心（支持 Webhook 直传与 2GB 扩展），并全面兼容 R2、S3、Discord 及 HuggingFace 等多存储后端。零成本构建你的全能私有数据金库。</sub>

---
## qianlong520/Telegram_MistRelay

> [!info]
> ⭐ 99 · Python · 2026-05-26T08:53:07Z  
> [GitHub](https://github.com/qianlong520/Telegram_MistRelay)  
> `#容器化` `#文件管理` `#自动化工具` 
> MistRelay 是一款基于 Telegram 机器人的 aria2 下载管理系统，支持自动下载并上传至 OneDrive，具备完整的数据校验机制确保文件安全。集成文件直链功能，支持 HTTP/磁力/种子下载。提供 Web 管理界面和 Windows 桌面客户端，支持 Docker 一键部署。
> <sub>自动化下载上传</sub>

---
## Panniantong/Agent-Reach

> [!info]
> ⭐ 38,540 · Python · 2026-06-23T20:22:20Z  
> [GitHub](https://github.com/Panniantong/Agent-Reach)  
> `#AI 智能体` `#网页爬虫` `#自动化工具` `#跨平台` `#agent-infrastructure` `#ai-agent` `#ai-search` `#automation` `#bilibili` `#claude-code` `#cli` `#cursor` `#free-api` `#llm-tools` `#mcp` `#python` `#reddit-scraper` `#twitter-scraper` `#web-scraper` `#xiaohongshu` `#youtube-transcript` 
> Agent Reach 是一款赋能 AI Agent 的开源 CLI 工具，赋予其全网信息获取能力。支持 Twitter、YouTube、小红书等主流平台，无需付费 API。通过多后端路由自动规避封锁，本地存储保障隐私，实现零成本、高可用的跨平台数据接入。
> <sub>Give your AI agent eyes to see the entire internet. Read & search Twitter, Reddit, YouTube, GitHub, Bilibili, XiaoHongShu — one CLI, zero API fees.</sub>

---
## shuaiplus/nodewarden

> [!info]
> ⭐ 2,831 · TypeScript · 2026-06-23T17:55:28Z  
> [GitHub](https://github.com/shuaiplus/nodewarden) · [Website](https://nodewarden.app)  
> `#PWA 应用` `#云端备份` `#密码管理` `#无服务器架构` `#bitwarden` `#cloudflare` `#cloudflare-d1` `#cloudflare-r2` `#cloudflare-workers` `#end-to-end-encryption` `#nodewarden` `#password` `#password-manager` `#preact` `#self-hosted` `#serverless` `#typescript` `#vaultwarden` `#web-vault` `#workers` `#zero-knowledge` 
> NodeWarden 是基于 Cloudflare Workers 的无服务器 Bitwarden 兼容服务端。核心亮点包括 PWA 离线访问、Passkey 无密码登录及 WebDAV/S3 云端备份。完美兼容官方客户端，支持 R2/KV 存储，是个人低成本自建密码管理系统的理想选择。
> <sub>Bitwarden-compatible server running on Cloudflare Workers</sub>

---
## qaz741wsd856/warden-worker

> [!info]
> ⭐ 1,149 · Rust · 2026-06-23T15:07:47Z  
> [GitHub](https://github.com/qaz741wsd856/warden-worker) · [Website](http://warden.qqnt.de/)  
> `#密码管理` `#无服务器` `#自托管` `#隐私安全` `#bitwarden` `#cloudflare-workers` `#password` `#password-manager` `#self-hosted` `#serverless` 
> Warden 是一个部署在 Cloudflare Workers 上的 Bitwarden 兼容服务器。它利用 D1 和 R2 实现免运维、零成本的自托管密码管理。支持核心密码库、TOTP 及实时同步，兼容官方客户端。适合追求隐私且希望降低运维负担的个人用户，实现“部署即忘”的轻量级安全方案。
> <sub>A Bitwarden-compatible server for Cloudflare Workers</sub>

---
## develop202/kgcheckin

> [!info]
> ⭐ 198 · JavaScript · 2026-06-23T04:52:24Z  
> [GitHub](https://github.com/develop202/kgcheckin)  
> `#API调用` `#GitHub Actions` `#自动化运维` `#自动签到` 
> 本项目基于 GitHub Actions 实现酷狗概念 VIP 的自动签到功能，支持二维码及手机号登录。通过调用 API 每日自动领取会员权益，提供详细的令牌配置与多账号管理指南，旨在探索自动化运维与 API 逆向技术，仅供学习研究使用。
> <sub>酷狗概念VIP自动签到</sub>

---
## XianYuDaXian/iptv-m3u-manager

> [!info]
> ⭐ 172 · HTML · 2026-06-18T21:23:15Z  
> [GitHub](https://github.com/XianYuDaXian/iptv-m3u-manager)  
> `#AI 视觉检测` `#IPTV 管理` `#自动化运维` `#订阅聚合` 
> IPTV M3U 管理器是一款强大的订阅聚合与过滤工具，支持多源合并、正则筛选及自定义分组。核心亮点在于集成 AI 视觉检测与自动化任务，可自动验证频道连通性、补齐 EPG 节目表，并提供 PC 与移动端双布局预览，实现 IPTV 资源的智能化管理与高效运维。
> <sub>IPTV M3U 管理器</sub>

---
## gaoyaxuan/luci-app-pushbot

> [!info]
> ⭐ 5 · Shell · 2026-05-27T08:40:12Z  
> [GitHub](https://github.com/gaoyaxuan/luci-app-pushbot)  
> `#OpenWRT` `#消息推送` `#网络运维` `#设备监控` 
> 该插件是 OpenWRT 路由器上的多功能通知工具，支持钉钉、企业微信、Bark 等主流平台。核心功能涵盖设备上下线告警、流量统计、IP 变动及系统状态监控。通过 LuCI 界面配置，实现路由器运行状态的实时推送与无人值守管理，提升运维效率。

---
## emojiiii/magekit-app

> [!info]
> ⭐ 15 · Rust · 2026-05-29T05:51:26Z  
> [GitHub](https://github.com/emojiiii/magekit-app)  
> `#Rust 开发` `#桌面应用` `#视频下载` 
> MageKit 是一款基于 Rust 和 GPUI 框架构建的现代化桌面视频下载器。它利用 yt-dlp 引擎支持 YouTube、Bilibili 等多平台视频获取，具备 GPU 加速界面、智能任务管理及元数据嵌入功能，提供跨平台的高性能下载体验。

---
## lesnolie/movecar

> [!info]
> ⭐ 1,142 · JavaScript · 2026-06-23T14:51:51Z  
> [GitHub](https://github.com/lesnolie/movecar)  
> `#无服务器` `#智能生活` `#自动化工具` `#隐私保护` 
> 这是一个基于 Cloudflare Workers 的无服务器挪车通知系统。它通过扫码实现双向位置共享，利用 Bark 推送保护车主隐私，避免电话泄露。支持防恶意骚扰机制，零运维成本，适合个人车主部署使用。
> <sub>🚗 智能挪车通知系统 - 基于 Cloudflare Workers，支持实时位置共享和多推送服务</sub>

---
## lostmaniac/air780e_forwarder

> [!info]
> ⭐ 38 · Lua · 2026-06-21T14:33:03Z  
> [GitHub](https://github.com/lostmaniac/air780e_forwarder)  
> `#LuatOS` `#消息通知` `#物联网` `#短信转发` 
> 基于 Air780E 模块与 LuatOS 的智能短信转发系统，支持企业微信、飞书、钉钉等多渠道推送。具备关键词与正则匹配过滤功能，采用异步协程处理，集成低功耗与容错机制，适用于物联网告警、远程验证码通知及自动化运维场景。

---
## jianzhichu/dysync.net

> [!info]
> ⭐ 174 · C# · 2026-06-21T10:42:55Z  
> [GitHub](https://github.com/jianzhichu/dysync.net)  
> `#.NET Core` `#Emby集成` `#媒体管理` `#视频同步` 
> dysync.net是一款基于.NET Core和Vue开发的抖音视频同步工具，支持多账号同步收藏夹、喜欢列表及指定博主作品。内置视频信息刮削功能，同步后可直接在Emby或Jellyfin媒体服务器中播放，提供Docker部署方案，适合NAS用户构建个人视频库。
> <sub>视频同步工具</sub>

---
## hezhizheng/go-wxpush

> [!info]
> ⭐ 1,657 · Go · 2026-06-23T10:03:27Z  
> [GitHub](https://github.com/hezhizheng/go-wxpush) · [Website](https://push.hzzio.top)  
> `#Go` `#微信服务` `#消息推送` 
> 基于 Golang 开发的极简免费微信消息推送服务，支持 Docker 一键部署（镜像仅 2MB），提供每日 10 万次推送额度，支持原生弹窗与声音提醒，具备多用户管理、自定义跳转页面及无限换肤功能，适用于个人及小型项目消息通知场景。
> <sub>极简且免费的微信消息推送服务 (基于golang)</sub>

---
## ieax/renewhelper

> [!info]
> ⭐ 356 · Vue · 2026-06-23T02:25:24Z  
> [GitHub](https://github.com/ieax/renewhelper) · [Website](https://lostfree.de5.net)  
> `#前端框架` `#无服务器` `#服务监控` `#运维自动化` `#icalendar` `#notify` `#renew` `#subscribe` 
> RenewHelper 是一款基于 Cloudflare Workers 的全栈服务生命周期管理工具。它支持订阅、域名和服务器到期提醒，无需服务器即可零成本托管。特色包括机甲风 UI、农历公历双历计算、多渠道通知推送及 iCal 同步。支持 Docker 和 Worker 部署，内置账单看板，是个人运维自动化的理想选择。
> <sub>RenewHelper - 时序·守望是一款基于Cloudflare Workers的全栈服务生命周期提醒、管理工具。它专为管理周期性订阅、域名续费、服务器到期等场景设计。无需服务器，零成本托管，提供精美的机甲风（Mecha-style）UI 界面、强大的农历/公历计算核心、多渠道通知推送能力、iCal日程同步以及账单管理功能。同时支持Worker和Docker方式部署。</sub>

---
## dushixiang/uart_sms_forwarder

> [!info]
> ⭐ 283 · TypeScript · 2026-06-23T01:22:08Z  
> [GitHub](https://github.com/dushixiang/uart_sms_forwarder)  
> `#Docker 部署` `#Lua 脚本` `#物联网` `#运维自动化` 
> 基于合宙 Air780 模块，通过 UART 串口实现短信转发与告警通知。支持钉钉、飞书等多平台集成，提供 Docker 与原生部署方案。适用于服务器监控、物联网设备通信及自动化运维场景，具备计划任务与来电通知功能，构建高效可靠的通信网关。
> <sub>Air780 短信UART转发器</sub>

---
## vodtv/api

> [!info]
> ⭐ 143 · JavaScript · 2026-06-23T19:05:08Z  
> [GitHub](https://github.com/vodtv/api) · [Website](https://mconfig.viptv.work/)  
> `#API代理` `#Cloudflare Workers` `#流媒体聚合` `#订阅管理` 
> MoonTV/LunaTV视频源配置与API中转工具。提供精简版订阅链接，支持Cloudflare Workers一键部署CORSAPI，实现被墙API自动中转与JSON配置转换。内置API状态自动检测机制，每小时监控可用性并生成健康报告，支持Base58编码订阅格式，提升流媒体资源访问稳定性。
> <sub>MoonTV/LunaTV源配置，每日自动检测API状态，可在CF部署CORSAPI中转被墙API，本人提供的CORSAPI仅为测试使用，请勿滥用！</sub>

---
## ngosang/trackerslist

> [!info]
> ⭐ 54,254 · N/A · 2026-06-23T19:19:15Z  
> [GitHub](https://github.com/ngosang/trackerslist) · [Website](https://ngosang.github.io/trackerslist/)  
> `#P2P下载` `#网络协议` `#自动化工具` `#bittorrent` `#bittorrent-tracker` `#bittorrent-trackers` `#http` `#list` `#lists` `#public-tracker` `#public-trackers` `#torrent` `#tracker` `#trackers` `#trackerslist` `#udp` `#webtorrent` `#ws` 
> 该仓库提供每日自动更新的公共 BitTorrent 追踪器列表，涵盖 UDP、HTTP、HTTPS 等多种协议。通过自动化脚本过滤无效节点，为用户提供高效、稳定的 P2P 下载配置源，显著提升下载成功率。
> <sub>Updated list of public BitTorrent trackers</sub>

---
## yonggekkk/argosbx

> [!info]
> ⭐ 5,331 · Shell · 2026-06-23T17:40:35Z  
> [GitHub](https://github.com/yonggekkk/argosbx) · [Website](https://yonggekkk.github.io/argosbx/)  
> `#Sing-box` `#一键部署` `#容器化部署` `#网络代理` `#anytls` `#argo` `#claw-cloud` `#cloudflared` `#docker-image` `#google-idx` `#hysteria2` `#ml-kem-768` `#sap` `#shadowsocks-2022` `#singbox` `#socks5` `#tuic` `#vless-reality-vision` `#vless-ws-tls` `#vless-xhttp-reality` `#vmess-ws` `#warp` `#wireguard` `#xray` 
> Argosbx 是一款专为小白设计的自建代理神器，支持 Sing-box、Xray 和 Argo 三内核自动搭配。它能在 VPS 和 Docker 环境中一键无交互部署，涵盖 Reality、Vless、Hysteria2 等十余种协议。内置 15 种 WARP 出站组合解锁流媒体，配合 CDN 方案实现高隐蔽性，是极简高效的网络加速工具。
> <sub>小白自建代理神器！ArgoSBX一键无交互小钢炮脚本💣：Sing-box、Xray、Argo三内核自动搭配；支持VPS、Docker、容器多环境部署；套CDN的5大方案+套WARP的15种组合；已支持协议：AnyTLS、Any-reality、Vless-xhttp-reality-vision-enc、Vless-tcp-reality-vision、Vless-xhttp-vision-enc、Vless-ws-vision-enc、Shadowsocks-2022、Hysteria2、Tuic、Socks5、Vmess-ws、Argo临时/固定隧道</sub>

---
## byJoey/cfnew

> [!info]
> ⭐ 14,270 · N/A · 2026-06-23T19:46:06Z  
> [GitHub](https://github.com/byJoey/cfnew)  
> `#Cloudflare Workers` `#网络代理工具` `#节点管理` `#订阅转换` 
> CFnew 是基于 Cloudflare Workers 的代理节点管理工具，支持 VLESS/Trojan/xhttp 多协议，内置订阅转换功能兼容 Clash/Surge/Sing-box 等主流客户端。提供图形化配置界面和 API 管理，支持延迟测试、地区筛选和 ECH 加密，配置实时生效无需重新部署。

---
## avwo/whistle

> [!info]
> ⭐ 15,570 · JavaScript · 2026-06-23T02:31:39Z  
> [GitHub](https://github.com/avwo/whistle) · [Website](https://wproxy.org/)  
> `#Node.js` `#网络抓包` `#调试代理` `#跨平台` `#charles` `#debug` `#fiddler` `#hosts` `#node` `#nodejs` `#proxy` `#web` `#weinre` 
> Whistle 是一款基于 Node.js 的跨平台网络抓包与调试代理工具。它支持 HTTP、HTTPS、HTTP/2 及 WebSocket 协议，允许用户通过规则灵活修改请求与响应。内置 Weinre、Console 等调试工具，并提供可视化界面与插件扩展能力，是 Web 开发、接口调试及网络分析的高效利器。
> <sub>HTTP, HTTP2, HTTPS, Websocket debugging proxy</sub>

---
## lanyeeee/bilibili-video-downloader

> [!info]
> ⭐ 1,536 · Rust · 2026-06-23T14:33:33Z  
> [GitHub](https://github.com/lanyeeee/bilibili-video-downloader)  
> `#Tauri 框架` `#媒体库管理` `#视频处理` `#跨平台应用` `#bilibili` `#download` `#downloader` `#gui` `#naive-ui` `#rust` `#tauri` `#tauri-app` `#vue` 
> 这是一款基于 Tauri v2 构建的跨平台哔哩哔哩视频下载器。支持番剧、课程等多类型视频及弹幕、字幕下载，具备 NFO 刮削与广告标记功能，可无缝对接 Emby 等媒体库，提供轻量高效的本地视频管理体验。
> <sub>哔哩哔哩 bilibili B站 视频 下载器，图形界面 + nfo刮削 + 广告标记 + 字幕下载 + 弹幕下载，轻松将视频加入emby等媒体库</sub>

---
## youhunwl/TVAPP

> [!info]
> ⭐ 18,751 · JavaScript · 2026-06-23T19:42:16Z  
> [GitHub](https://github.com/youhunwl/TVAPP) · [Website](https://app.iyouhun.com)  
> `#Android TV` `#家庭影音` `#资源聚合` `#android` `#android-tv` `#apk` `#app` `#tv` `#tv-box` 
> 该仓库聚合全网 Android TV 电视盒子优质应用，涵盖影视、直播、K 歌及工具类 APK。提供便捷下载、自动更新及兼容性标注，支持 TVBox 等影音壳接口配置。旨在通过安全验证与分类索引，助力用户快速搭建家庭影音娱乐中心。
> <sub>收集全网 Android TV电视盒子应用，涵盖影视、直播、K歌、工具、游戏等类型，整理优质APK资源，支持便捷下载与自动更新。提供安全验证、分类索引与兼容性标注，助力用户打造家庭影音娱乐中心！              ✅ TVBox/影视仓等影音壳接口配置源。</sub>

---
## xyfqzy/free-nodes

> [!info]
> ⭐ 123 · Python · 2026-06-23T19:51:34Z  
> [GitHub](https://github.com/xyfqzy/free-nodes)  
> `#科学上网` `#网络代理` `#自动化运维` `#订阅管理` 
> 该仓库聚合了 V2Ray、Shadowsocks、Clash 等主流协议的免费代理节点订阅链接。通过 GitHub Actions 实现每 6 小时自动更新，确保节点高速稳定。提供多平台客户端推荐及 API 接口，是用户获取科学上网资源、实现跨网络访问的便捷工具，支持一键导入与自动化运维。
> <sub>🚀 免费代理节点大全 | V2Ray SS Trojan Clash 订阅链接 | 科学上网工具 | 翻墙软件 | 免费VPN | 代理服务器 | 免费梯子 | 机场节点 | Free Proxy Nodes | Shadowsocks | Vmess | Vless | 每日更新 | 高速稳定</sub>

---
## hyue418/go-rename

> [!info]
> ⭐ 17 · Go · 2026-03-26T02:50:05Z  
> [GitHub](https://github.com/hyue418/go-rename)  
> `#Go` `#批量重命名` `#文件管理` 
> go-rename 是一款基于 Golang 开发的跨平台照片与视频批量重命名工具。支持依据 EXIF 拍摄时间、创建/修改时间及 MD5 值进行智能重命名与去重，自动处理文件名冲突，有效解决多设备素材整理难题，适用于 Windows、MacOS 及 Linux 系统。
> <sub>go-rename 照片视频重命名工具 支持Windows/MacOS/Linux等各平台</sub>

---
## btjawa/BiliTools

> [!info]
> ⭐ 5,241 · Rust · 2026-06-23T15:16:49Z  
> [GitHub](https://github.com/btjawa/BiliTools) · [Website](https://btjawa.top/bilitools)  
> `#Tauri 框架` `#多媒体处理` `#视频下载` `#跨平台应用` `#bilibili` `#download` `#rust` `#tauri-app` `#typescript` `#vue` 
> BiliTools 是一款基于 Tauri 构建的跨平台哔哩哔哩工具箱，支持视频、音频、弹幕及元数据下载。集成 FFmpeg 与 aria2，提供高清资源解析、AI 总结及多格式转换，具备扫码登录与风控验证功能，打造轻量高效的 B 站资源管理方案。
> <sub>A cross-platform bilibili toolbox. 跨平台哔哩哔哩工具箱，支持下载视频、番剧等等各类资源</sub>

---
## GitHubDaily/GitHubDaily

> [!info]
> ⭐ 46,857 · N/A · 2026-06-23T19:55:36Z  
> [GitHub](https://github.com/GitHubDaily/GitHubDaily) · [Website](https://githubdaily.com)  
> `#AI 大模型` `#开发者工具` `#开源资讯` `#技术聚合` `#ai` `#algorithms-and-data-structures` `#backend` `#developer-tools` `#development` `#frontend` `#github` `#java` `#javascript` `#kubernetes` `#linux` `#markdown` `#open-source` `#python` `#tutorials` `#web` 
> GitHubDaily 是一个坚持分享高质量开源项目的技术资讯平台，覆盖 AI 工具、开发框架及实用资源。通过聚合精选项目与年度复盘，帮助开发者快速掌握技术趋势，提升编程能力，是获取前沿开源资讯与学习路径的权威指南。
> <sub>坚持分享 GitHub 上高质量、有趣实用的开源技术教程、开发者工具、编程网站、技术资讯。A list cool, interesting projects of GitHub.</sub>

---
## proxifly/free-proxy-list

> [!info]
> ⭐ 5,743 · N/A · 2026-06-23T19:53:10Z  
> [GitHub](https://github.com/proxifly/free-proxy-list) · [Website](https://proxifly.dev)  
> `#代理服务器` `#爬虫开发` `#网络工具` `#自动化更新` `#free-proxy` `#free-proxy-list` `#fresh-proxies` `#http` `#http-proxy` `#proxies` `#proxies-list` `#proxy` `#proxy-list` `#proxy-scraper` `#socks` `#socks4` `#socks4-proxy` `#socks5` `#socks5-proxy` 
> Proxifly 维护的免费代理列表仓库，每 5 分钟自动更新。提供 HTTP、SOCKS4/5 等协议，覆盖 94 国超 3500 个节点。支持 JSON/TXT/CSV 多格式及 NPM 模块调用，是爬虫开发、网络测试及隐私保护的优质基础设施。
> <sub>🚀 Free HTTP, SOCKS4, & SOCKS5 Proxy List * Updated every 5 minutes *</sub>

---
## 1c7/chinese-independent-developer

> [!info]
> ⭐ 48,943 · N/A · 2026-06-23T19:53:23Z  
> [GitHub](https://github.com/1c7/chinese-independent-developer)  
> `#AI应用` `#工具产品` `#独立开发` `#china` `#indie` `#indie-developer` 
> 中国独立开发者项目列表聚合了2026年2月最新上线的创新产品，涵盖AI语音助手、投资理财工具、打字练习平台、音乐生成、简历优化、浏览器插件等多个领域。项目以网站或App形式呈现，强调实用性与用户体验，如智能语音朗读、美股A股基金定投计算、AI音乐创作等，展现了中国独立开发者在AI应用与垂直场景工具上的活跃创新。
> <sub>👩🏿‍💻👨🏾‍💻👩🏼‍💻👨🏽‍💻👩🏻‍💻中国独立开发者项目列表 -- 分享大家都在做什么</sub>

---
## fish2018/pansou

> [!info]
> ⭐ 13,624 · Go · 2026-06-23T19:50:37Z  
> [GitHub](https://github.com/fish2018/pansou) · [Website](https://so.252035.xyz/)  
> `#API 服务` `#异步架构` `#插件系统` `#网盘搜索` 
> PanSou 是一款高性能网盘资源搜索 API，支持 TG 频道与自定义插件并发搜索。通过异步机制与多级缓存优化响应速度，自动识别多种网盘类型并智能排序。提供 Docker 一键部署方案，支持 JWT 认证，适合构建私有资源检索系统。
> <sub>PanSou是一款高性能的网盘资源搜索API服务，支持TG频道和插件搜索。系统设计以性能和可扩展性为核心，支持多频道多插件并发搜索、结果智能排序和网盘类型分类。docker集成前后端，一键启动，开箱即用。仅供学习研究，请勿以各种形式用于盈利目的。</sub>

---
## Usagi-org/ai-goofish-monitor

> [!info]
> ⭐ 12,646 · Python · 2026-06-23T20:02:07Z  
> [GitHub](https://github.com/Usagi-org/ai-goofish-monitor)  
> `#AI智能体` `#电商监控` `#网页爬虫` `#自动化运维` `#ai` `#ai-assistant` `#ai-tools` `#automation` `#gemini` `#goofish` `#open-source` `#openai` `#playwright` `#tool` `#xian-yu` `#xianyu` `#xianyu-bot` 
> 基于 Playwright 与 AI 大模型的闲鱼智能监控系统，提供 Web 可视化后台。支持多任务并发、自然语言创建任务及多模态商品分析，具备定时调度、账号代理轮换及多渠道即时通知功能，帮助用户高效筛选心仪商品。
> <sub>基于 Playwright 和AI实现的闲鱼多任务实时/定时监控与智能分析系统，配备了功能完善的后台管理UI。帮助用户从闲鱼海量商品中，找到心仪产品。</sub>

---
## ring1012/r1-iot-java

> [!info]
> ⭐ 75 · Java · 2026-06-10T08:11:19Z  
> [GitHub](https://github.com/ring1012/r1-iot-java)  
> `#Java` `#容器化` `#物联网` `#运维自动化` 
> 该项目基于Docker容器化技术，结合Java开发环境，为斐讯R1等智能设备提供物联网解决方案。通过一键部署脚本实现设备自动化运维，支持配置持久化与跨平台运行，适用于智能家居系统的快速搭建与设备管理。

---
## OpenListTeam/OpenList

> [!info]
> ⭐ 23,207 · Go · 2026-06-23T15:57:15Z  
> [GitHub](https://github.com/OpenListTeam/OpenList) · [Website](https://doc.oplist.org)  
> `#WebDAV` `#云存储管理` `#开源治理` `#文件挂载` `#alist` `#aliyunpan` `#baidupan` `#openlist` 
> OpenList 是 AList 的社区驱动分支，旨在应对开源信任危机。它支持阿里云盘、OneDrive 等数十种存储后端，提供统一的 WebDAV 挂载与文件管理。项目坚持 AGPL-3.0 协议，强调代码透明与长期治理，是构建去中心化存储架构的理想选择。
> <sub>A new AList Fork to Anti Trust Crisis</sub>

---
## TapXWorld/ChinaTextbook

> [!info]
> ⭐ 74,486 · Roff · 2026-06-23T17:11:17Z  
> [GitHub](https://github.com/TapXWorld/ChinaTextbook)  
> `#PDF教材` `#义务教育` `#教育公平` `#教育资源` 
> 该项目开源了国内小初高及大学PDF教材资源，旨在促进教育公平与普及，消除地区间教育贫困。特别聚焦小学数学和初中数学教材，提供人教版义务教育教科书，支持海外华人子女了解国内教育体系，推动以兴趣为导向的学习。
> <sub>所有小初高、大学PDF教材。</sub>

---
## krau/SaveAny-Bot

> [!info]
> ⭐ 2,382 · Go · 2026-06-23T08:51:09Z  
> [GitHub](https://github.com/krau/SaveAny-Bot) · [Website](https://sabot.unv.app)  
> `#Telegram 机器人` `#数据归档` `#自动化运维` `#跨平台存储` `#alist` `#downloader` `#pikpak` `#telegram-bot` `#webdav` 
> SaveAny-Bot 是一款强大的 Telegram 文件管理工具，支持将文件保存至 Alist、S3、WebDAV 等多种存储后端。它不仅能绕过 Telegram 的内容限制，还集成了 yt-dlp 和 Aria2，支持批量下载、流式传输及 JS 插件扩展，实现跨平台数据归档与自动化运维。
> <sub>Save Any Telegram File to Anywhere 📂 (Alist, Disk, Webdav, S3, Rclone...) . Support restrict saving content and files beyond telegram.</sub>

---
## yt-dlp/yt-dlp

> [!info]
> ⭐ 172,880 · Python · 2026-06-23T20:19:51Z  
> [GitHub](https://github.com/yt-dlp/yt-dlp) · [Website](https://discord.gg/H5MNcFW63r)  
> `#命令行工具` `#媒体处理` `#音视频下载` `#cli` `#downloader` `#python` `#sponsorblock` `#youtube-dl` `#youtube-downloader` `#yt-dlp` 
> yt-dlp 是一款功能强大的命令行音视频下载工具，作为 youtube-dl 的活跃分支，它支持全球数千个视频网站。具备格式优选、元数据编辑及插件扩展能力，支持 SponsorBlock 跳过片头。它是媒体归档、离线存储及自动化数据采集的首选方案，性能卓越且持续更新。
> <sub>A feature-rich command-line audio/video downloader</sub>

---
## thinkoaa/Deadpool

> [!info]
> ⭐ 698 · Go · 2026-06-23T02:04:03Z  
> [GitHub](https://github.com/thinkoaa/Deadpool)  
> `#Go语言` `#代理池` `#流量转发` `#网络安全` 
> Deadpool 是一款基于 Go 开发的高效 Socks5 代理池工具，支持从 Hunter、Quake、FOFA 等测绘平台自动获取高质量代理，并具备本地导入、轮询转发及健康检查功能。适用于渗透测试、流量转发及 IP 轮换场景，通过配置文件灵活管理代理资源，保障网络请求的匿名性与稳定性。
> <sub>deadpool代理池工具，可从hunter、quake、fofa等网络空间测绘平台取高质量socks5代理，或本地导入socks5代理，轮询使用代理进行流量转发。</sub>

---
## BCUninstaller/Bulk-Crap-Uninstaller

> [!info]
> ⭐ 19,850 · C# · 2026-06-23T17:41:25Z  
> [GitHub](https://github.com/BCUninstaller/Bulk-Crap-Uninstaller) · [Website](https://www.bcuninstaller.com/)  
> `#卸载工具` `#系统运维` `#自动化工具` `#application-manager` `#application-uninstaller` `#bloatware` `#bulk-actions` `#bulk-uninstall` `#cleaner` `#debloat` `#disk-space` `#inno-setup` `#msi` `#nsis` `#portableapps` `#software-management` `#unattended-uninstall` `#uninstall` `#uninstaller` `#windows` `#windows-10` `#windows-forms` 
> Bulk Crap Uninstaller 是一款强大的开源批量卸载工具，专为高效清理 Windows 系统而设计。它支持一键移除大量应用、清理残留文件及检测孤立程序，兼容 Windows Store、Steam 及多种安装框架。作为 Apache 2.0 协议项目，它免费供个人和商业使用，是 IT 运维和系统维护的必备利器。
> <sub>Remove large amounts of unwanted applications quickly.</sub>

---
## imsyy/DailyHotApi

> [!info]
> ⭐ 3,885 · TypeScript · 2026-06-23T16:17:35Z  
> [GitHub](https://github.com/imsyy/DailyHotApi)  
> `#API 接口` `#数据聚合` `#热点追踪` `#api` `#daily-hot` `#list` `#rss` `#vercel` 
> DailyHotApi 是一个高效的全网热门数据聚合 API 接口，支持哔哩哔哩、微博、知乎等主流平台。提供 JSON 与 RSS 双模式，响应极快，支持 Vercel 一键部署，是开发者获取实时热点数据的理想工具。
> <sub>🔥 今日热榜 API，一个聚合热门数据的 API 接口，支持 RSS 模式 及 Vercel 部署 | 前端页面：https://github.com/imsyy/DailyHot</sub>

---
## TheSpeedX/PROXY-List

> [!info]
> ⭐ 5,647 · N/A · 2026-06-23T19:57:40Z  
> [GitHub](https://github.com/TheSpeedX/PROXY-List)  
> `#代理服务器` `#数据抓取` `#网络安全` `#隐私保护` `#anonymity` `#anonymous` `#elite` `#free` `#free-proxy` `#hacking` `#http` `#https-proxy` `#proxy` `#proxy-list` `#socker` `#socks` `#socks-proxy` `#socks4-proxy` `#socks5-proxy` `#speedx` `#vpn` 
> 该仓库提供每日自动更新的免费公共代理列表，涵盖 HTTP、SOCKS4 及 SOCKS5 协议。通过 GitHub 托管原始数据，支持开发者快速集成至爬虫或网络测试工具中，旨在辅助网络数据采集与隐私保护，强调教育用途。
> <sub>Get PROXY List  that gets updated everyday</sub>

---
## sunny9577/proxy-scraper

> [!info]
> ⭐ 583 · JavaScript · 2026-06-23T18:42:59Z  
> [GitHub](https://github.com/sunny9577/proxy-scraper) · [Website](https://sunny9577.github.io/proxy-scraper/)  
> `#Node.js` `#代理服务器` `#浏览器自动化` `#网络爬虫` `#angularjs` `#freeproxy` `#getproxy` `#http-proxy` `#https-proxy` `#nodejs` `#protractor` `#proxies` `#proxy` `#proxy-list` `#proxy-scraper` `#proxylist` `#scraper` `#selenium` `#socks` `#socks5` `#socks5-proxy` 
> 基于 Protractor 和 Node.js 的自动化代理抓取工具，利用浏览器自动化技术提取高可用代理 IP。支持 HTTP、SOCKS4/5 等协议，提供 TXT、JSON 等五种数据格式。列表每 3 小时自动更新，包含 1700+ 实时节点，适用于网络爬虫与隐私保护。
> <sub>⭐️ A proxy scraper made using Protractor | Proxy list Updates every three hour 🔥</sub>

---
## adminouyang/231006

> [!info]
> ⭐ 236 · JavaScript · 2026-06-23T20:20:08Z  
> [GitHub](https://github.com/adminouyang/231006)  
> `#家庭娱乐系统` `#资源聚合` `#配置管理` 
> 该仓库专注于TVBox智能电视盒子的配置文件收集与整理，提供多样化的媒体资源链接和插件配置方案，帮助用户快速搭建个性化家庭娱乐系统。通过结构化分类和定期更新，简化用户部署流程，提升使用体验。
> <sub>主要搜集tvbox配置文件</sub>

---
## lyswhut/lx-music-mobile

> [!info]
> ⭐ 17,261 · TypeScript · 2026-06-23T18:23:44Z  
> [GitHub](https://github.com/lyswhut/lx-music-mobile) · [Website](https://lyswhut.github.io/lx-music-doc/)  
> `#React Native` `#开源软件` `#数据同步` `#音乐播放器` `#javascript` `#music-player` `#react-native-app` 
> LX Music 移动版是一款基于 React Native 和 Redux 构建的 Android 音乐播放器。支持自定义音乐源，能从公开接口拉取数据，并提供私有化数据同步服务。遵循 Apache 2.0 协议，注重版权合规，适合进阶用户进行个性化音乐管理与跨端同步。
> <sub>一个基于 React native 开发的音乐软件</sub>

---
## breezy-weather/breezy-weather

> [!info]
> ⭐ 10,471 · Kotlin · 2026-06-23T17:19:38Z  
> [GitHub](https://github.com/breezy-weather/breezy-weather)  
> `#Jetpack Compose` `#Kotlin` `#数据可视化` `#生活工具` 
> Breezy Weather 是一款免费基于 Kotlin 与 Jetpack Compose 开发的开源 Android 气象应用。它聚合超 50 个数据源，提供预报、空气质量等全方位数据。采用 Material 3 Expressive 设计，界面美观交互流畅，是追求数据丰富度与现代化体验的安卓用户首选工具。
> <sub>A feature-rich weather app with good visualizations and more than 50 sources.</sub>

---
## guoyue2010/lxmusic-

> [!info]
> ⭐ 4,389 · JavaScript · 2026-06-23T15:49:45Z  
> [GitHub](https://github.com/guoyue2010/lxmusic-) · [Website](https://github.com/guoyue2010/lxmusic-/releases)  
> `#API集成` `#音乐流媒体` `#音源订阅` 
> 本项目是洛雪音乐与澜音的第三方音源聚合平台，提供全网最新最全的音乐资源接口。核心功能包括免费与付费音源订阅服务，支持多平台导入配置。项目强调版权合规，建议用户支持正版，并提供完善的社群支持与购买渠道，是音乐爱好者获取高品质音源的实用工具。
> <sub>lxmusic(洛雪音乐)全网最新最全音源</sub>

---
## lalifeier/IPTV

> [!info]
> ⭐ 391 · Python · 2026-06-23T18:07:53Z  
> [GitHub](https://github.com/lalifeier/IPTV) · [Website](https://iptv-ten-dun.vercel.app)  
> `#IPTV 直播源` `#流媒体服务` `#跨平台兼容` 
> 本项目聚合了全网高清 IPTV 直播源，提供 TXT 与 M3U 双格式订阅链接，支持自动更新。覆盖央视、卫视及主流直播平台，兼容安卓电视、Apple TV、Jellyfin 及 Windows 播放器。内置 XMLTV 电子节目单，实现跨平台无缝观看，是家庭流媒体搭建的必备工具。

---
## cngege/GitHubDesktop2Chinese

> [!info]
> ⭐ 713 · C++ · 2026-06-23T12:50:50Z  
> [GitHub](https://github.com/cngege/GitHubDesktop2Chinese)  
> `#C++` `#GitHub 生态` `#本地化工具` `#桌面应用` `#chinese-simplified` `#chinese-translation` `#cpp` `#githubdesktop` 
> GitHubDesktop2Chinese 是一款专为 GitHub Desktop 客户端打造的自动化本地化工具。通过 C++ 开发结合 JSON 映射机制，实现文本精准替换与版本兼容。支持自动检测更新与社区协作翻译，有效解决中文用户界面语言障碍，提升开发体验。
> <sub>GithubDesktop语言本地化(汉化)工具 【GitHub桌面客户端中文汉化】</sub>

---
## Evil0ctal/Douyin_TikTok_Download_API

> [!info]
> ⭐ 18,461 · Python · 2026-06-23T17:41:20Z  
> [GitHub](https://github.com/Evil0ctal/Douyin_TikTok_Download_API) · [Website](https://douyin.wtf)  
> `#API接口` `#异步爬虫` `#社交媒体数据` `#api` `#async` `#crawler` `#douyin` `#douyin-api` `#douyin-scraper` `#douyin-tiktok-api` `#douyin-tiktok-download` `#fastapi` `#no-watermark` `#online-parsing` `#python` `#pywebio` `#scraper` `#spider` `#tiktok` `#tiktok-api` `#tiktok-scraper` `#tiktok-signature` `#web-scraping` 
> 这是一个高性能异步数据爬取工具，支持抖音、TikTok、快手及Bilibili等多平台。它提供开箱即用的API接口，支持在线批量解析与视频下载，适用于社交媒体数据采集与分析场景，具备高并发处理能力，是开发者构建爬虫应用的理想选择。
> <sub>🚀「Douyin_TikTok_Download_API」是一个开箱即用的高性能异步抖音、快手、TikTok、Bilibili数据爬取工具，支持API调用，在线批量解析及下载。</sub>

---
## cxasm/notepad--

> [!info]
> ⭐ 9,741 · C++ · 2026-06-23T14:51:37Z  
> [GitHub](https://github.com/cxasm/notepad--)  
> `#国产替代` `#文本编辑器` `#跨平台应用` 
> Notepad-- 是一款由国人开发的跨平台文本编辑器，支持 Windows、Linux 和 Mac 系统。其核心目标是实现国产信创环境下的软件替代，特别是在 Uos 系统上表现优异。项目致力于提供轻量、高效的编辑体验，旨在减少对外部软件的依赖，是国产基础软件生态的重要补充。
> <sub>一个支持windows/linux/mac的文本编辑器，目标是做中国人自己的编辑器，来自中国。</sub>

---
## 521xueweihan/HelloGitHub

> [!info]
> ⭐ 162,453 · Python · 2026-06-23T19:50:13Z  
> [GitHub](https://github.com/521xueweihan/HelloGitHub) · [Website](https://hellogithub.com)  
> `#开发者工具` `#开源社区` `#项目推荐` `#awesome` `#github` `#hellogithub` `#python` 
> HelloGitHub 是一个专注于分享 GitHub 上有趣、入门级开源项目的月刊，每月28日定期发布，涵盖有趣项目、开源书籍、实战及企业级项目。旨在通过精选优质开源内容，帮助用户快速发现开源魅力，激发学习兴趣，降低入门门槛，从而爱上开源生态。
> <sub>:octocat: 分享 GitHub 上有趣、入门级的开源项目。Share interesting, entry-level open source projects on GitHub.</sub>

---
## bia-pain-bache/BPB-Worker-Panel

> [!info]
> ⭐ 12,126 · TypeScript · 2026-06-23T17:21:14Z  
> [GitHub](https://github.com/bia-pain-bache/BPB-Worker-Panel) · [Website](https://bia-pain-bache.github.io/BPB-Worker-Panel/)  
> `#Cloudflare Workers` `#网络代理` `#订阅生成` `#amnezia` `#android` `#chain` `#clash-core` `#doh-server` `#fragment` `#ios` `#linux` `#mihomo` `#proxy-chain` `#singbox-core` `#trojan` `#vless` `#warp` `#windows` `#xray-core` 
> BPB-Worker-Panel 是一个基于 Cloudflare Workers 的 GUI 面板，提供 VLESS、Trojan 和 Warp 协议的免费订阅生成。它支持私有 DoH 服务器、链式代理、Fragment 分片及丰富的路由规则，兼容 Xray、Sing-box 等主流客户端，旨在为用户提供安全、私密且跨平台的网络配置解决方案。
> <sub>A GUI Panel providing Worker subscriptions for VLESS, Trojan and Warp configs alongside a private DoH server and chain proxies, offering full DNS, clean IP,  Fragment, Warp, Warp pro and routing settings for cross-platform clients using Amnezia, Wireguard, Sing-box, Clash/Mihomo and Xray cores.</sub>

---
## VirtualHotBar/HotPEToolBox

> [!info]
> ⭐ 2,046 · Batchfile · 2026-06-23T16:10:01Z  
> [GitHub](https://github.com/VirtualHotBar/HotPEToolBox) · [Website](https://www.hotpe.top)  
> `#WinPE` `#故障排查` `#系统运维` `#自动化构建` 
> HotPEToolBox 是一个强大的 WinPE 构建工具，支持 Win10/11 镜像定制。它提供纯净、优雅的系统维护环境，具备网络、远程、BitLocker 解锁等高级功能。通过 Python 脚本自动化生成 ISO，兼容 UEFI/Legacy 启动，是系统运维和故障排查的理想选择。
> <sub>一个纯净、强大、优雅的WinPE</sub>

---
## VirtualHotBar/NetMount

> [!info]
> ⭐ 1,377 · TypeScript · 2026-06-23T12:34:23Z  
> [GitHub](https://github.com/VirtualHotBar/NetMount) · [Website](http://netmount.cn)  
> `#Tauri 框架` `#云存储管理` `#跨平台桌面应用` 
> NetMount 是一款基于 Rust 和 Tauri 开发的跨平台桌面应用，旨在统一管理和挂载各类云存储设施。通过集成 Rclone 和 OpenList 技术，为用户提供高效、安全的云存储访问体验，支持多平台部署与便捷的文件管理。
> <sub>统一管理和挂载云存储设施/Unified management and mounting of cloud storage facilities</sub>

---
## ZCShou/GoGoGo

> [!info]
> ⭐ 10,729 · Java · 2026-06-23T13:03:01Z  
> [GitHub](https://github.com/ZCShou/GoGoGo) · [Website](https://itexp.blog.csdn.net/)  
> `#Android 开发` `#位置模拟` `#地图集成` `#虚拟定位` `#android` `#fake-gps` `#fake-locations` `#java` `#joystick` `#map` `#mock-location` `#mock-locations` `#mockgps` `#virtual-location` 
> 影梭是一款基于 Android 调试 API 与百度地图 SDK 开发的虚拟定位工具。它无需 ROOT 权限即可修改位置，并提供摇杆模拟移动功能。项目开源 GPL-3.0，旨在辅助开发者学习 Android 定位机制及地图应用集成，严禁用于作弊。
> <sub>一个基于 Android 调试 API + 百度地图实现的虚拟定位工具，并且同时实现了一个可以自由移动的摇杆</sub>

---
## Guovin/iptv-api

> [!info]
> ⭐ 24,351 · Python · 2026-06-23T19:52:51Z  
> [GitHub](https://github.com/Guovin/iptv-api)  
> `#数据采集` `#跨平台` `#运维自动化` `#auto-update` `#awesome` `#epg` `#iptv` `#iptv-api` `#iptv-channels` `#iptv-free` `#iptv-m3u` `#iptv-m3u8` `#iptv-playlist` `#ipv4` `#ipv6` `#live` `#m3u` `#m3u8` `#playlist` `#rtmp` `#schedule` `#tv` `#tvbox` 
> IPTV直播源自动更新平台，实现采集、筛选、测速全流程自动化。支持自定义频道、IPv4/IPv6双栈及推流优化弱网体验。提供 Docker、CLI 及 GUI 多端部署方案，集成 EPG 与台标管理，是构建稳定 IPTV 系统的核心工具。
> <sub>⚡️IPTV直播源自动更新平台『🤖全自动采集、筛选、测速、生成🚀』：自定义获取频道；IPv4/IPv6；接口验效；推流；多平台支持；工作流、Docker、命令行、GUI多端部署 | IPTV live streaming source automatic update platform</sub>

---
## yonggekkk/Cloudflare-vless-trojan

> [!info]
> ⭐ 15,421 · JavaScript · 2026-06-23T16:58:43Z  
> [GitHub](https://github.com/yonggekkk/Cloudflare-vless-trojan) · [Website](https://ygkkk.blogspot.com/2023/07/cfworkers-vless.html)  
> `#Cloudflare Workers` `#代理脚本` `#加密代理` `#网络代理` `#argo` `#cdn` `#clash-meta` `#cloudflare` `#cloudflare-pages` `#cloudflare-workers` `#ech` `#http` `#nat64` `#reality` `#sing-box` `#socks5` `#trojan` `#vless` `#xray` `#ygkkk` 
> 基于Cloudflare Workers/Pages的免费代理脚本，支持VLESS/Trojan协议及Socks5/Http本地代理。提供ECH-TLS、普通TLS、无TLS三种加密模式，支持自定义域名、优选IP和ProxyIP配置，适合搭建稳定高效的网络代理服务。
> <sub>CF-workers/pages代理脚本：支持Vless-ws(tls)、Trojan-ws(tls)；Socks5/http本地代理脚本：可选ECH-TLS、普通TLS、无TLS三种代理模式</sub>

---
## dr34m-cn/taosync

> [!info]
> ⭐ 1,494 · Python · 2026-06-23T14:56:23Z  
> [GitHub](https://github.com/dr34m-cn/taosync)  
> `#数据备份` `#网盘管理` `#自动化同步` 
> TaoSync 是一款专为 OpenList/AList v3+ 设计的自动化文件同步工具。它支持跨平台部署与 Docker 运行，提供全同步、仅新增等多种模式，实现本地与网盘间的高效备份。具备可视化进度监控、定时任务及通知功能，完全离线运行，确保用户隐私安全。
> <sub>TaoSync是一个适用于OpenList v3+的自动化同步工具/Sync for OpenList/AList</sub>

---
## 1234567Yang/cf-proxy-ex

> [!info]
> ⭐ 1,800 · JavaScript · 2026-06-23T05:50:43Z  
> [GitHub](https://github.com/1234567Yang/cf-proxy-ex) · [Website](https://y.demo.lhyang.org/)  
> `#AI 工具接入` `#Cloudflare Workers` `#无服务器架构` `#网络代理` `#bypass` `#cloudflare-proxy` `#cloudflare-reverse-proxy` `#cloudflare-workers` `#free-proxy` `#github-proxy` `#goguardian` `#linewize` `#online-proxy` `#reverse-proxy` 
> 本项目基于 Cloudflare Workers 构建无服务器代理，支持多平台部署。核心亮点在于集成 Duckduckgo AI 聊天接口，兼容 GPT-4o/Claude3，并提供 Github 加速及 Libgen 解锁功能。具备 Cookie 作用域管理与表单提交能力，确保访问体验接近原生，是高效、安全的网络访问解决方案。
> <sub>Cloudflare超级代理，无服务器代理，Duckduckgo代理（可用AI聊天，包含GPT4o/Claude3），Github加速，支持解锁Libgen，在线代理。现已支持多平台部署。Cloudflare super proxy, setting up a free serverless proxy by using Cloudflare worker, support Duckduckgo / Libgen. Now you can deploy this project on different platforms.</sub>

---
## peasoft/NoMoreWalls

> [!info]
> ⭐ 3,336 · Python · 2026-06-23T19:46:05Z  
> [GitHub](https://github.com/peasoft/NoMoreWalls) · [Website](https://ghproxy.net/https://raw.githubusercontent.com/peasoft/NoMoreWalls/master/list.meta.yml)  
> `#网络代理` `#自动化脚本` `#节点订阅` `#bypass-gfw` `#china` `#clash` `#gfw` `#gfw-breaker` `#hysteria` `#hysteria2` `#node` `#proxy` `#shadowrocket` `#shadowsocks` `#socks` `#socks5` `#ss` `#ssr` `#trojan` `#tuic` `#v2ray` `#vless` `#vmess` 
> NoMoreWalls 是开源自动化节点聚合工具，利用 GitHub Actions 实时抓取并合并互联网公开代理节点。支持 Clash Meta 与 Sing-Box 订阅，内置智能分流规则，为用户提供免费、无需注册的网络安全访问方案，倡导反 996 劳动权益。
> <sub>自动抓取合并互联网上的公开节点。</sub>

---
## qist/tvbox

> [!info]
> ⭐ 10,003 · JavaScript · 2026-06-23T15:18:07Z  
> [GitHub](https://github.com/qist/tvbox)  
> `#家庭娱乐` `#流媒体聚合` `#脚本自动化` `#配置管理` 
> 本仓库提供 TVBox、OK影视及猫影视等多平台的聚合配置文件，涵盖直播、点播及解析源。支持 GitHub/Gitee 托管与私有仓库部署，集成多种第三方资源接口与网盘授权配置，旨在为用户提供灵活、可定制的家庭娱乐与流媒体播放解决方案。
> <sub>OK影视、tvbox配置文件，如果喜欢，请Fork自用。使用前请仔细阅读仓库说明，一旦使用将被视为你已了解。</sub>

---
## NextAlone/Nagram

> [!info]
> ⭐ 2,903 · Java · 2026-06-23T11:04:55Z  
> [GitHub](https://github.com/NextAlone/Nagram)  
> `#Android 应用` `#即时通讯` `#网络代理` `#隐私工具` 
> Nagram 是一款基于 NekoX 深度定制的第三方 Telegram Android 客户端。它提供了丰富的增强功能，包括多协议代理支持（VMess/SSR/Trojan）、高级消息管理（合并/编辑/强制复制）、国际化翻译及隐私优化。适用于追求更高自由度、隐私保护和网络连通性的 Telegram 高级用户。
> <sub>The third-party Telegram android app.</sub>

---
## httpsok/httpsok

> [!info]
> ⭐ 1,258 · Shell · 2026-06-23T17:44:28Z  
> [GitHub](https://github.com/httpsok/httpsok) · [Website](https://httpsok.com/doc/)  
> `#SSL证书管理` `#自动化工具` `#运维自动化` `#nginx` `#ssl` `#ssl-certificate` `#ssl-certificates` `#ssl-support` `#tls` `#tls-certificate` 
> httpsok 是一款高效的 SSL 证书自动续期工具，通过一行命令即可实现证书自动检测、续签及多云厂商 CDN 管理。支持 Nginx、Apache、泛域名、Docker 环境及失效告警，极大简化运维流程，是保障 Web 安全与自动化管理的得力助手。
> <sub>🔥🔥🔥一行命令，轻松搞定SSL证书自动续期。 SSL泛域名证书，SSL证书管理，SSL证书自动续期，SSL证书自动更新，七牛云、阿里云、腾讯云、CDN、OSS、https证书管理,https证书管理,免费SSL证书,TLS证书自动续签,nginx证书自动续期工具，nginx证书管理。从此告别Let's Encrypt</sub>

---
## zhuima/awesome-cloudflare

> [!info]
> ⭐ 14,192 · N/A · 2026-06-23T17:49:08Z  
> [GitHub](https://github.com/zhuima/awesome-cloudflare) · [Website](https://cloudflare.chuhai.tools/)  
> `#Cloudflare生态` `#云基础设施` `#开源资源聚合` `#独立开发工具` 
> 专为独立开发者打造的 Cloudflare 生态资源库，精选图床、临时邮箱及监控等开源工具。利用 Workers、R2 等免费服务实现全栈部署，显著降低运维成本与开发门槛。持续更新精选指南与最佳实践，助力项目快速落地与高效运维。
> <sub>⛅️ 精选的 Cloudflare 工具、开源项目、指南、博客和其他资源列表。/ ⛅️ A curated list of Cloudflare tools, open source projects, guides, blogs and other resources.</sub>

---
## sandboxie-plus/Sandboxie

> [!info]
> ⭐ 18,691 · C · 2026-06-23T17:59:04Z  
> [GitHub](https://github.com/sandboxie-plus/Sandboxie) · [Website](https://Sandboxie-Plus.com)  
> `#沙箱隔离` `#系统安全` `#软件测试` 
> Sandboxie Plus 是一款 Windows 系统级沙箱隔离工具，通过创建安全环境运行程序，实现非侵入式安装与测试。支持无限沙箱并行，Plus 版提供 Qt 界面、快照管理及隐私控制，适用于未知程序测试、安全浏览及系统维护，保障主机安全。
> <sub>Sandboxie Plus & Classic</sub>

---
## spiritLHLS/ecs

> [!info]
> ⭐ 6,966 · Shell · 2026-06-23T10:17:46Z  
> [GitHub](https://github.com/spiritLHLS/ecs) · [Website](https://t.me/+UHVoo2U4VyA5NTQ1)  
> `#VPS性能分析` `#服务器测评` `#跨平台` `#运维自动化` `#almalinux` `#arch` `#astralinux` `#bench-script` `#benchmark` `#cdn` `#centos` `#checker` `#debian` `#fedora` `#goecs` `#ipv6` `#lemonbench` `#openai` `#oracle-linux` `#rockylinux` `#speedtest` `#sysbench` `#ubuntu` `#vps` 
> VPS融合怪服务器测评项目，提供全面的VPS性能基准测试与网络质量分析。支持多系统架构，推荐使用无环境依赖的Go版本，具备非root权限运行能力，可一键修复系统原生Bug，是运维人员评估服务器性能与网络回程路由的必备工具。
> <sub>VPS 融合怪服务器测评项目 更推荐使用无环境依赖的Go版本 VPS Fusion Monster Server Test Script – More recommended to use the Go version with no environment dependencies: https://github.com/oneclickvirt/ecs</sub>

---
## wanghongenpin/proxypin

> [!info]
> ⭐ 13,362 · Dart · 2026-06-23T17:39:33Z  
> [GitHub](https://github.com/wanghongenpin/proxypin)  
> `#Flutter` `#流量分析` `#网络抓包` `#跨平台` `#capture-traffic` `#charles` `#fiddler` `#proxy` `#proxypin` `#zhuabao` 
> ProxyPin 是一款开源免费的跨平台 HTTP(S) 抓包工具，支持 Windows、Mac、Android、iOS 及 Linux。基于 Flutter 构建，具备扫码连接、流量拦截、脚本处理及请求重写等核心功能，适用于移动应用调试与网络分析。
> <sub>Open source free capture HTTP(S) traffic  software ProxyPin, supporting full platform systems</sub>

---
## Barabama/FreeNodes

> [!info]
> ⭐ 2,864 · Python · 2026-06-23T10:38:32Z  
> [GitHub](https://github.com/Barabama/FreeNodes)  
> `#网络工具` `#网页爬虫` `#自动化工具` `#节点聚合` `#clash` `#proxy` `#ss` `#trojan` `#v2ray` `#vless` `#vmess` 
> FreeNodes是一个AI驱动的免费代理节点爬虫工具，自动聚合v2ray和Clash订阅链接。每日定时运行，支持多源采集与镜像加速，提供txt/yaml双格式输出，适用于网络学习与技术交流场景。
> <sub>v2ray、clash免费节点爬虫，自动更新</sub>

---
## mfbpn/mfbpn_nodes

> [!info]
> ⭐ 27 · Python · 2026-06-05T12:25:32Z  
> [GitHub](https://github.com/mfbpn/mfbpn_nodes)  
> `#GitHub Actions` `#网络工具` `#自动化脚本` `#节点订阅` 
> 本仓库基于 GitHub Actions 实现免费代理节点的自动化测速与筛选，支持 SS、Vmess、Trojan 等多协议。通过 LiteSpeedTest 和 subconverter 生成高速稳定的 Base64 及 Clash 订阅链接，为用户提供可靠的网络加速资源，并支持自定义配置与 PR 贡献。

---
## alantang1977/X

> [!info]
> ⭐ 661 · Python · 2026-06-23T06:50:39Z  
> [GitHub](https://github.com/alantang1977/X)  
> `#流媒体技术` `#电视盒子` `#资源聚合` `#配置接口` 
> 本仓库是一个影视资源聚合导航站，核心提供 TVBox 配置接口、IPTV 直播源及影视 APP 下载链接。集成音乐下载、临时邮箱等实用工具，并分享 Cloudflare 搭建与 Github 加速教程。内容仅供学习研究，严禁商用及境内使用，用户需自行承担风险。
> <sub>如果喜欢，请点亮右上角星星，如有侵权请反馈！谢谢！免责声明：📛本仓库管理者不能保证本仓库内容的合法性、准确性、完整性和有效性，请根据情况自行判断。本仓库内容，仅用于测试和学习研究，禁止用于商业用途，不得将其用于违反国家、地区、组织等的法律法规或相关规定的其他用途，禁止任何公众号、自媒体进行任何形式的转载、发布，请不要在中华人民共和国境内使用本仓库内容，否则后果自负</sub>

---
## 8680/GOODBYEADS

> [!info]
> ⭐ 1,801 · Python · 2026-06-23T17:33:13Z  
> [GitHub](https://github.com/8680/GOODBYEADS)  
> `#DNS 拦截` `#去广告规则` `#广告屏蔽` `#网络工具` `#adblock` `#adblock-list` `#adblock-plus` `#adguard` `#adguard-list` `#adguardhome` `#quantumult-x` 
> GOODBYEADS 是一个聚合型去广告规则集合，专为 AdGuard、Quantumult X 和 SmartDNS 设计。它自动合并并去重多个优质上游源，提供海量拦截规则与白名单。支持定时更新与多格式订阅，旨在为用户提供高效、纯净的网络浏览体验，是网络隐私保护与广告屏蔽的强力工具。
> <sub>适用于AdGuard、Quantumult X、SmartDNS的去广告规则，合并优质上游规则并去重整理排列。</sub>

---
## n3rddd/N3RD

> [!info]
> ⭐ 75 · JavaScript · 2026-06-23T15:55:40Z  
> [GitHub](https://github.com/n3rddd/N3RD)  
> `#AI智能体` `#数据可视化` `#跨平台` 
> CTV项目聚焦智能电视生态的AI解决方案，通过多模态交互技术实现内容推荐与用户行为分析。核心应用涵盖跨平台视频服务优化及实时数据可视化，采用轻量化模型部署方案提升终端响应效率。
> <sub>CTV</sub>

---
## 217heidai/adblockfilters

> [!info]
> ⭐ 7,167 · Python · 2026-06-23T16:56:47Z  
> [GitHub](https://github.com/217heidai/adblockfilters)  
> `#DNS过滤` `#广告过滤` `#规则自动化` `#adblock` `#adguard` `#adguardhome` `#clash` `#dnsmasq` `#fiters` `#hosts` `#invizible` `#loon` `#mihomo` `#mosdns` `#personaldnsfilter` `#quantumultx` `#shadowrocket` `#sing-box` `#smartdns` 
> 这是一个自动化的去广告规则合并项目，每8小时定时更新。通过多源合并、去重及DNS解析验证，去除无效域名后生成优化规则库。提供AdGuard、DNSMasq等多种格式，支持国内外DNS服务，含Lite轻量版本，适配浏览器插件与DNS过滤工具。
> <sub>去广告合并规则，每8个小时更新一次。</sub>

---
## JoeanAmier/TikTokDownloader

> [!info]
> ⭐ 14,873 · Python · 2026-06-23T17:33:19Z  
> [GitHub](https://github.com/JoeanAmier/TikTokDownloader) · [Website](https://discord.com/invite/ZYtmgKud9Y)  
> `#Python` `#批量下载` `#抖音/TikTok` `#数据采集` `#abogus` `#api` `#csv` `#docker` `#douyin` `#downloader` `#ffmpeg` `#httpx` `#linux` `#macos` `#mstoken` `#pyinstaller` `#python` `#rich` `#server` `#sqlite` `#tiktok` `#ttwid` `#windows` `#xlsx` 
> DouK-Downloader 是一款基于 Python 和 HTTPX 的免费开源数据采集工具，支持批量下载抖音/TikTok 视频、图集、直播、合集及评论数据。具备 Web UI、API 接口、多线程下载、代理支持、增量采集、文件断点续传等功能，可部署至私有或公开服务器，适用于内容创作者、数据分析师等场景。
> <sub>TikTok 发布/喜欢/合辑/直播/视频/图集/音乐；抖音发布/喜欢/收藏/收藏夹/视频/图集/实况/直播/音乐/合集/评论/账号/搜索/热榜数据采集工具/下载工具</sub>

---
## yonggekkk/x-ui-yg

> [!info]
> ⭐ 3,811 · Shell · 2026-06-23T17:58:16Z  
> [GitHub](https://github.com/yonggekkk/x-ui-yg) · [Website](https://ygkkk.blogspot.com/2023/05/reality-xui-chatgpt.html)  
> `#Sing-box` `#Xray` `#流量管理` `#网络代理` `#alpine` `#argo` `#chatgpt` `#clash-meta` `#cloudflared` `#httpupgrade` `#oblivion` `#psiphon` `#sing-box` `#socks5` `#warp` `#wireguard` `#x-ui` `#xhttp` `#xray-core` 
> 本项目是 x-ui 的精简修改版脚本，支持 IPV4/6 及多架构部署。核心亮点在于集成 Xhttp 传输、MLDSA65 加密及 Argo 双隧道技术，内置 Psiphon 30 国分流功能。支持 Sing-box 与 Clash-meta 聚合订阅，提供本地化节点生成，确保数据安全，是高性能代理搭建与流量管理的优选工具。
> <sub>x-ui精简修改版脚本，支持最新的Xhttp传输协议、ENC加密、MLDSA65加密，集成argo固定临时双隧道（可共存）、Psiphon赛风VPN（30个国家）分流功能，支持部分节点聚合订阅、sing-box订阅、clash-meta订阅的配置输出</sub>

---
## mediago-dev/mediago

> [!info]
> ⭐ 9,031 · TypeScript · 2026-06-23T14:21:14Z  
> [GitHub](https://github.com/mediago-dev/mediago) · [Website](https://downloader.caorushizi.cn/guides.html?form=github)  
> `#AI智能体` `#网页爬虫` `#视频处理` `#跨平台` `#bilibili` `#downloader` `#electron` `#hls` `#m3u8` `#m3u8-downloader` `#m3u8download` `#video` `#youtube` 
> MediaGo 是一款跨平台视频下载工具，支持桌面端、Docker 及浏览器插件。内置流媒体嗅探，轻松抓取 m3u8/HLS 资源，依托 yt-dlp 支持 YouTube、Bilibili 等千余站点。特色在于集成 AI 助手技能，支持自然语言指令下载，并提供 HTTP API 与内置转码，实现采集到处理的全流程自动化。
> <sub>Cross-platform video downloader — sniff and grab m3u8/HLS streams, Bilibili, YouTube and more. Desktop + Docker + 🦞.</sub>

---
## biliup/biliup

> [!info]
> ⭐ 5,225 · Rust · 2026-06-23T18:05:15Z  
> [GitHub](https://github.com/biliup/biliup) · [Website](https://biliup.github.io/biliup/)  
> `#AI 智能体` `#Rust 后端` `#内容创作自动化` `#直播录制` `#bilibili` `#douyin` `#douyu` `#huya` `#twitch` `#youtube` 
> biliup 是一个强大的 B 站自动化工具，支持直播录制、视频上传和跨平台搬运。它采用 Rust 后端加 Next.js 前端的混合架构，提供 WebUI 和命令行接口。支持多种登录方式，可实现 24x7 无人值守运行，并能作为 AI Agent 技能集成，是 UP 主自动化的首选方案。
> <sub>自动直播录制、投稿、twitch、ytb频道搬运工具。命令行投稿(B站)和视频下载工具，提供多种登录方式，支持多p。</sub>

---
## lyswhut/lx-music-sync-server

> [!info]
> ⭐ 809 · TypeScript · 2026-06-19T06:39:02Z  
> [GitHub](https://github.com/lyswhut/lx-music-sync-server)  
> `#Node.js` `#数据同步` `#私有化部署` `#运维自动化` `#nodejs` 
> 基于 Node.js 的 LX Music 私有数据同步服务端，支持收藏列表跨设备实时同步。需配合 Nginx 反向代理实现 HTTPS 安全传输，内置 PM2 进程管理与数据快照恢复。适合私有化部署，确保音乐数据自主可控与多端一致。
> <sub>运行在 Node.js 上的 LX Music 数据同步服务</sub>

---
## 1Panel-dev/1Panel

> [!info]
> ⭐ 35,966 · Go · 2026-06-23T18:17:24Z  
> [GitHub](https://github.com/1Panel-dev/1Panel) · [Website](https://1panel.pro)  
> `#AI智能体` `#VPS控制面板` `#容器管理` `#运维自动化` `#agent` `#clawdbot` `#copaw` `#docker` `#docker-ui` `#hermes` `#hermes-agent` `#linux` `#lnmp` `#ollama` `#openclaw` `#openresty` `#qwenpaw` `#webmin` 
> 1Panel 是一款现代化的开源 VPS 控制面板，也是唯一原生支持 AI Agent 的面板。它允许用户通过简洁的 Web 界面运行 Ollama 模型、部署 OpenClaw Agent，并管理整个服务器栈。无需记忆 CLI，支持一键部署网站、自动 SSL 和 Docker 容器管理。
> <sub>🔥 1Panel is a modern, open-source VPS control panel — and the only one with native AI agent support. Run Ollama models, deploy OpenClaw agents, and manage your entire server stack from one clean web interface.</sub>

---
## privacy-protection-tools/anti-AD

> [!info]
> ⭐ 10,502 · N/A · 2026-06-23T11:03:35Z  
> [GitHub](https://github.com/privacy-protection-tools/anti-AD) · [Website](https://anti-ad.net)  
> `#DNS 过滤` `#网络组件` `#隐私保护` `#adblock` `#adguardhome` `#dnsmasq` `#easylist` `#neohosts` `#php` `#pi-hole` `#surge` 
> anti-AD 是中文区高命中率广告过滤列表，通过 DNS 解析层精准拦截广告与隐私追踪。兼容 AdGuardHome、Clash、Surge 等主流组件，支持多格式输出。经过去重与模糊匹配优化，有效屏蔽电视及 App 内置广告，保护个人隐私，完全开源。
> <sub>致力于成为中文区命中率最高的广告过滤列表，实现精确的广告屏蔽和隐私保护。anti-AD 现已支持 AdGuardHome，dnsmasq，Surge，Pi-Hole，Clash, mihomo，smartdns，sing-box 等网络组件。完全兼容常见的广告过滤工具所支持的各种广告过滤列表格式</sub>

---
## yuliskov/SmartTube

> [!info]
> ⭐ 30,726 · Java · 2026-06-23T17:10:44Z  
> [GitHub](https://github.com/yuliskov/SmartTube) · [Website](https://smarttubeapp.github.io)  
> `#开源媒体客户端` `#无广告播放` `#智能电视应用` `#android` `#android-tv` `#android-tv-box` `#java` `#kotlin` `#retrofit2` `#rxjava-android` 
> SmartTube 是一款专为 Android TV 打造的开源媒体客户端，支持 8K 高清播放与 SponsorBlock 广告过滤。它无需 Google 服务即可运行，提供高度可定制的界面与播放控制，兼容老旧设备。项目近期修复了开发环境安全问题，致力于为用户提供纯净、安全的电视端视频浏览体验。
> <sub>Browse media content with your own rules on Android TV</sub>

---
## leiurayer/downkyi

> [!info]
> ⭐ 24,405 · N/A · 2026-06-23T16:19:50Z  
> [GitHub](https://github.com/leiurayer/downkyi)  
> `#B站工具` `#多媒体处理` `#视频下载` `#跨平台` 
> 哔哩下载姬(downkyi)是一款专为哔哩哔哩设计的视频下载工具，支持批量下载及8K、HDR、杜比视界等高清格式。内置工具箱提供音视频提取、去水印等实用功能，是B站用户获取高清视频资源的高效解决方案。
> <sub>哔哩下载姬downkyi，哔哩哔哩网站视频下载工具，支持批量下载，支持8K、HDR、杜比视界，提供工具箱（音视频提取、去水印等）。</sub>

---
## dgtlmoon/changedetection.io

> [!info]
> ⭐ 32,104 · Python · 2026-06-23T16:36:10Z  
> [GitHub](https://github.com/dgtlmoon/changedetection.io) · [Website](https://changedetection.io)  
> `#AI智能体` `#Web监控` `#变更检测` `#网页爬虫` `#back-in-stock` `#change-alert` `#change-detection` `#change-monitoring` `#monitoring` `#notifications` `#restock-monitor` `#rss` `#self-hosted` `#url-monitor` `#web-scraping` `#website-change-detection` `#website-change-detector` `#website-change-monitor` `#website-change-notification` `#website-change-tracker` `#website-defacement-monitoring` `#website-monitor` `#website-monitoring` `#website-watcher` 
> changedetection.io 是一款强大的网站变更监控工具，支持实时检测网页内容变化并通过多种渠道发送警报。它集成了 AI 大模型能力，能智能过滤无关变更并生成自然语言摘要，极大降低误报。支持 Docker 自托管和 SaaS 服务，适用于价格追踪、库存监控及网页防篡改等场景，是运维和数据分析的高效利器。
> <sub>Best and simplest tool for website change detection, web page monitoring, and website change alerts. Perfect for tracking content changes, price drops, restock alerts, and website defacement monitoring—all for free or enjoy our SaaS plan!</sub>

---
## fangzesheng/free-api

> [!info]
> ⭐ 16,118 · N/A · 2026-06-23T00:52:52Z  
> [GitHub](https://github.com/fangzesheng/free-api)  
> `#API 聚合` `#免费接口` `#开发辅助` `#api` `#freeapi` 
> 该仓库致力于收集并整理全网免费 API 接口，涵盖 AI 大模型、生活查询及工具类服务。每月定期更新，提供包括 DeepSeek、通义千问等热门模型接口及天气、彩票等实用数据，旨在降低开发者调用成本，打造一站式免费 API 资源库。
> <sub>收集免费的接口服务,做一个api的搬运工</sub>

---
## easychen/rsspush

> [!info]
> ⭐ 627 · N/A · 2026-06-18T17:14:41Z  
> [GitHub](https://github.com/easychen/rsspush)  
> `#Docker部署` `#消息推送` `#自动化运维` `#资讯聚合` `#apprise` `#docker` `#docker-compose` `#rss` `#rsshub` `#serverchan` 
> RSSPush 是一款基于 Docker 部署的 Web 应用，专注于监测 RSS 源变动并自动推送至微信、Telegram 等数十种消息通道。支持关键词过滤、自动翻译及 RSSHub 集成，提供便捷的配置界面与一键安装脚本，是个人资讯聚合与自动化通知的理想工具。
> <sub>监测RSS变动，并发送最新内容到微信、Webhook 和 Telegram, Discord, Slack, Amazon SNS, Gotify 等数十个消息通道。</sub>

---
## LogicJake/WebMonitor

> [!info]
> ⭐ 910 · Python · 2026-06-16T17:31:55Z  
> [GitHub](https://github.com/LogicJake/WebMonitor) · [Website](https://logicjake.github.io/WebMonitor/#/)  
> `#Web 监控` `#可视化 UI` `#网页爬虫` `#自动化通知` `#monitor` `#rss` `#web-monitor` 
> WebMonitor 是一款功能强大的网页实时监控工具，支持 PhantomJS 抓取异步内容，提供 XPath、CSS 及 JsonPath 解析能力。内置邮件、微信、Telegram 等多渠道通知，具备可视化 UI 界面，适用于运维监控及信息追踪场景。
> <sub>实时监控网页变化，并发送通知（Monitor web page changes in real time and send notifications）</sub>

---
## XIU2/TrackersListCollection

> [!info]
> ⭐ 31,594 · N/A · 2026-06-23T16:51:01Z  
> [GitHub](https://github.com/XIU2/TrackersListCollection) · [Website](https://trackerslist.com)  
> `#P2P 下载` `#网络加速` `#资源聚合` `#aria2` `#aria2-format-tracker` `#bittorrent` `#bittorrent-trackers` `#qbittorrent` `#torrent` `#torrent-tracker` `#tracker` `#trackers` `#trackerslist` `#utorrent` 
> 这是一个每天自动更新的热门 BT Tracker 列表集合。它通过聚合全网优质 Tracker 服务器，帮助用户显著提升 BitTorrent 下载速度和连接稳定性。支持 Aria2 等多种客户端配置，提供 best/all 等多种分类列表，是 P2P 下载用户的必备工具。
> <sub>🎈 Updated daily! A list of popular BitTorrent Trackers! / 每天更新！全网热门 BT Tracker 列表！</sub>

---
## filecxx/FileCentipede

> [!info]
> ⭐ 10,890 · C++ · 2026-06-23T16:04:34Z  
> [GitHub](https://github.com/filecxx/FileCentipede) · [Website](http://filecxx.com)  
> `#下载管理` `#多协议支持` `#文件传输` `#跨平台` `#bittorrent-client` `#bt` `#download` `#download-manager` `#download-videos` `#ftp-client` `#http-client` `#libtorrent` `#m3u8` `#magnet` `#qt` `#remote-download` `#ssh-client` `#stream-downloader` `#torrent` `#transmission` `#video-downloader` `#webdav-client` 
> FileCentipede 是一款跨平台全能文件传输管理器，支持 HTTP、FTP、SSH、BitTorrent 及磁力链接等多协议。它集成了 WebDAV 客户端、浏览器插件及资源探索器，提供视频下载、文件合并与加密等辅助工具，旨在为用户提供高效、安全且高度可定制的文件上传下载解决方案。
> <sub>Cross-platform internet upload/download manager for HTTP(S), FTP(S), SSH, magnet-link, BitTorrent, m3u8, ed2k, and online videos.  WebDAV client, FTP client, SSH client.</sub>

---
## hongyonghan/Docker_Microsoft365_E5_Renew_X

> [!info]
> ⭐ 1,375 · Dockerfile · 2026-06-14T03:08:38Z  
> [GitHub](https://github.com/hongyonghan/Docker_Microsoft365_E5_Renew_X)  
> `#Docker容器化` `#云资源管理` `#自动化运维` `#订阅续订` 
> 本项目提供基于 Docker 的 Microsoft 365 E5 订阅自动续订服务。支持多 CPU 架构部署，镜像轻量仅 225M，具备邮件通知与 Web 管理界面。通过容器化封装简化配置，实现跨平台自动化运维，适合个人及 NAS 用户学习 Docker 部署与 API 调用。
> <sub>Docker版本的E5调用API续订服务：Microsoft 365 E5 Renew X</sub>

---
## lyswhut/lx-music-desktop

> [!info]
> ⭐ 51,745 · TypeScript · 2026-06-23T18:19:02Z  
> [GitHub](https://github.com/lyswhut/lx-music-desktop) · [Website](https://lyswhut.github.io/lx-music-doc/)  
> `#Electron` `#桌面应用` `#跨平台` `#音乐播放器` `#electron` `#javascript` `#music-player` `#vue` 
> LX Music 是基于 Electron 30+ 与 Vue 3 构建的跨平台桌面音乐播放器，支持 Win/macOS/Linux。核心亮点在于开放 HTTP API 接口，支持第三方调用及 Scheme URL 联动，配合独立同步服务实现多端数据无缝同步。软件强调高度可定制与快捷键操作，适合进阶用户打造个性化音乐体验。
> <sub>一个基于 Electron 的音乐软件</sub>

---
## BluePointLilac/ContextMenuManager

> [!info]
> ⭐ 19,396 · C# · 2026-06-23T18:01:00Z  
> [GitHub](https://github.com/BluePointLilac/ContextMenuManager) · [Website](https://bluepointlilac.github.io/ContextMenuManager)  
> `#.NET` `#右键菜单` `#系统工具` 
> ContextMenuManager 是一款专为 Windows 打造的纯粹右键菜单管理工具。它支持对文件、文件夹菜单项进行精细化控制，包括启用/禁用、修改属性及自定义添加。程序兼容多版本 Windows 系统，采用非暴力隐藏机制，有效避免系统残留，是系统优化与运维管理的高效工具。
> <sub>🖱️ 纯粹的Windows右键菜单管理程序</sub>

---
## bclswl0827/v2ray-heroku

> [!info]
> ⭐ 2,056 · Shell · 2026-04-25T16:58:51Z  
> [GitHub](https://github.com/bclswl0827/v2ray-heroku) · [Website](https://ibcl.us/Heroku-V2Ray_20191014/)  
> `#V2Ray` `#云端部署` `#网络代理` `#v2ray-heroku` `#websocket-v2ray` 
> 本仓库提供在 Heroku 平台一键部署 V2Ray WebSocket 代理的解决方案。支持自动更新镜像，确保使用最新版本。通过绑定域名或 CloudFlare Workers 反向代理，可有效提升连接速度与稳定性。适合需要在云端快速搭建代理服务的用户，操作简便且资源占用合理。
> <sub>用于在 Heroku 上部署 V2Ray WebSocket。</sub>

---
## ShareX/ShareX

> [!info]
> ⭐ 38,245 · C# · 2026-06-23T18:50:02Z  
> [GitHub](https://github.com/ShareX/ShareX) · [Website](https://getsharex.com)  
> `#桌面应用` `#自动化工具` `#运维自动化` `#avalonia` `#capture` `#color-picker` `#csharp` `#dropbox` `#file-sharing` `#file-upload` `#ftp` `#gif` `#gif-recorder` `#image-annotation` `#ocr` `#productivity` `#region-capture` `#screen-capture` `#screen-recorder` `#screenshot` `#share` `#sharex` `#url-shortener` 
> ShareX 是一款免费开源的 Windows 全能截图与文件共享工具。它支持一键捕获屏幕任意区域、录制视频，并能将内容上传至多种云存储或自定义目的地。内置强大的图像处理、OCR 识别及自动化工作流，是提升个人与团队生产力的必备神器。
> <sub>ShareX is a free and open-source application that enables users to capture or record any area of their screen with a single keystroke. It also supports uploading images, text, and various file types to a wide range of destinations.</sub>

---
## zhaoolee/ChromeAppHeroes

> [!info]
> ⭐ 25,664 · JavaScript · 2026-06-23T12:53:13Z  
> [GitHub](https://github.com/zhaoolee/ChromeAppHeroes) · [Website](https://zhaoolee.com/ChromeAppHeroes/)  
> `#插件推荐` `#浏览器插件` `#自动化工具` 
> 谷粒-Chrome插件英雄榜是一个开源项目，为优质Chrome插件提供中文说明书，帮助用户高效发现和使用翻译、去广告、视频处理等实用工具。基于docsify构建的免费文档平台，持续更新且完全开源，旨在降低插件使用门槛，提升日常浏览与学习体验，造福广大网民。
> <sub>🌈谷粒-Chrome插件英雄榜, 为优秀的Chrome插件写一本中文说明书, 让Chrome插件英雄们造福人类~  ChromePluginHeroes, Write a Chinese manual for the excellent Chrome plugin, let the Chrome plugin heroes benefit the human~ 公众号「0加1」同步更新</sub>

---
## 2dust/v2rayN

> [!info]
> ⭐ 109,765 · C# · 2026-06-23T19:52:34Z  
> [GitHub](https://github.com/2dust/v2rayN) · [Website](https://v2rayn.2dust.link)  
> `#图形化客户端` `#多内核支持` `#网络代理` `#跨平台` `#proxy` `#shadowsocks` `#socks5` `#trojan` `#v2fly` `#v2ray` `#vless` `#vmess` `#windows` `#xray` `#xtls` 
> v2rayN 是一款支持 Windows、Linux 和 macOS 的跨平台图形化客户端，兼容 Xray、sing-box 等多种核心内核。提供 GPG 签名校验保障安全，是用户进行网络代理配置与管理的高效工具。
> <sub>A GUI client for Windows, Linux and macOS, support Xray and sing-box and others</sub>

---
